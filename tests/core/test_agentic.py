...
from auth_scan.core.attack_surface import AttackSurfaceModel, AuthEndpoint, ModelToken
...
class TestObserve:
    ...
    def test_observe_extracts_endpoints(self) -> None:
        ...
        def run():
            ...
            result = ModuleResult(findings=[Finding(
        ...
        def run():
            ...
            class FakeJwtModule:
                ...
                def run(self, target, http_client, report, config):
                    call_count[0] += 1
                    return ModuleResult(findings=[Finding(
                        title="JWT Found",
                        severity=Severity.INFO,
                        evidence={"algorithm": "HS256", "location": "cookie"},
                        module_name="jwt",
