...
class MfaBypass(BaseAttackModule):
    ...
    def run(
        ...
        # Test bypasses
        self._find_mfa_endpoints(report, target)
    ...
    def _detect_mfa(self, report: ScanReport) -> list[str]:
        ...
        probe_headers = {
            k.lower(): str(v).lower() for k, v in report.metadata.get("probe_headers", {}).items()
        }
    ...
    def _find_mfa_endpoints(self, report: ScanReport, target: str) -> list[str]:
        ...
        for match in re.finditer(
            r'(?:href|action)=["\']([^"\']*(?:mfa|2fa|verify|otp|two-factor)[^"\']*)["\']',
            probe_body,
            re.I,
        ):
